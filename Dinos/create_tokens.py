...
def create_new_image(number_of_images, config):
    new_image = {}
    ...
    for incomp in config["incompatibilities"]:
        for attr in new_image:
            if new_image[incomp["layer"]] == incomp["value"] and new_image[attr] in incomp["incompatible_with"]:
                return create_new_image(number_of_images, config)

    if new_image in number_of_images:
        return create_new_image(number_of_images, config)
    else:
        return new_image
...
if __name__ == "__main__":
    with open('layers.json') as layer_file:
        layers = json.load(layer_file)
        generate_unique_images(10000, layers)

# Additional layer objects can be added following the above formats. They will automatically be composed along with
# the rest of the layers as long as they are the same size as eachother.
